# 이 파일은 현재 사용되지 않습니다. scheduling_main.py를 직접 사용하세요.
# 모든 실행 코드는 주석 처리되어 있습니다.
...
# --------- Step 1: 파일 로드와 파싱 ---------
# 이 코드는 현재 사용되지 않습니다. scheduling_main.py를 직접 사용하세요.
...
if not split_documents:
    print("[INFO] 분할된 문서가 없습니다. FAISS 인덱스를 생성하지 않습니다.")
    vectorstore = None
    retriever = None
else:
    print(f"[INFO] {len(split_documents)}개의 문서 청크를 찾았습니다.")
    # 실제 사용 시에는 환경변수에서 설정을 가져와야 합니다
    vectorstore = None
    retriever = None
...
print("[INFO] scheduling/__init__.py는 현재 사용되지 않습니다. scheduling_main.py를 직접 사용하세요.")
